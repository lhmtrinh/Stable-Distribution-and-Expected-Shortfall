...
function pdf = stab(x, a, b)
    ...
    if (x == 0) % Borak et al. (2005)
        ...
        tol = 1e-9; display = 0;
        integ = quadv(@integrand, -t0, pi/2, tol, display, abs(x), a, t0); pdf = a*x^(1 / (a-1)) / pi / abs(a-1)* integ;
    end

function I = integrand(t , x, a, t0)
    ct = cos(t); s = t0 + t;
    v = (cos(a*t0))^(1/(a-1))*(ct/sin(a*s))^(a/(a-1))*(cos(a*s - t)/ct); term = -x^(a / (a-1)); I = v .* exp(term*v);
